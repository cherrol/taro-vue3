...
import { setStorageSync, getStorageSync, login, checkSession, getUserProfile, navigateTo } from "@tarojs/taro";
		...
		wx.showModal({
			...
			success(res) {
				...
				if (res.cancel) {
					...
					reject(new Error("您未授权获取微信个人信息，部分功能可能无法正常使用！"));
				}
			},
			fail() {
				Toast.fail("您未授权获取微信个人信息，部分功能可能无法正常使用！");
	...
	try {
		...
		if (!(isAppLogin && userInfo !== "" && cookie !== "" && token !== "")) {
			...
			if (userInfo === "") {
				...
				if (!appProfileRes) {
					throw new Error("您未授权获取微信个人信息，部分功能可能无法正常使用！");
				}
		...
		const currentRole = adminProfile.data.roles.some((role) => role.id === "lawyer")
			? "lawyer"
